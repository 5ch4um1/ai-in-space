...
def search_images(
    ...
    window_seconds: int = Query(default=30 * 24 * 60 * 60, description="Search window in seconds"),
...
def get_image(
    ...
    lat: float = Query(..., description="Latitude"),
    timestamp: str = Query(..., description="ISO timestamp"),
    spectral_bands: str = Query(default="red,green,blue", description="Comma-separated bands"),
    size_km: float = Query(default=5.0, description="Image size in km"),
    window_seconds: int = Query(default=10 * 24 * 60 * 60, description="Search window in seconds")
):
    ...
    if not item:
        raise HTTPException(status_code=404, detail="No image found for the given location and time")
    ...
    if len(bands) == 1:
        ...
        if not all([r_asset, g_asset, b_asset]):
            raise HTTPException(status_code=400, detail="One or more bands not available")
        
        with COGReader(r_asset.href) as r_cog, \
